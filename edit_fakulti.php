...
if (isset($_POST['search'])) {
	...
	$sql = "SELECT * FROM fakulti WHERE fakulti_id = '$searchValue'";
} else {
...
<body>
	...
	<form method="post" style="text-align:center">
		<label for="search">Search Fakulti ID:</label>
		<input type="text" id="search" name="search" placeholder="Masukkan Fakulti ID">
		...
		<tbody>
			...
			if (mysqli_num_rows($result) > 0) {
				...
				while($row = mysqli_fetch_assoc($result)) {
					echo "<tr>";
					echo "<td>" . $row["fakulti_id"] . "</td>";
					echo "<td>" . $row["nama_fakulti"] . "</td>";
                    echo "<td>" . $row["alamat_fakulti"] . "</td>";
					echo "<td><a href='ubah_fakulti.php?fakulti_id=" . $row["fakulti_id"] . "'><button>Ubah</button></a></td>";
					echo "<td><a href='delete_fakulti.php?id=" . $row["fakulti_id"] . "'><button>Padam</button></a></td>";
					echo "</tr>";
				}
			} else {
  ...
  <a href="fakulti.php"><button style="position: fixed; bottom: 20px; right: 20px;">Kembali</button></a>
  <a href="edit_fakulti.php"><button style="position: fixed; bottom: 20px; right: 120px;">Muat Semula</button></a>
  <script>
